...
<title>Hardwood Draft — The Draft Program</title>
...
.picktag{text-align:center;font-family:'Anton';font-size:13px;letter-spacing:2px;padding:7px;
  background:var(--ink);color:var(--paper);text-transform:uppercase;transition:background .14s}
.pcard:hover .picktag{background:var(--orange)}
  ...
  if(challengers.length===0){
    ...
    challengers.map((ch,ci)=>{
      const chips=ch.players.map(p=>{const c=tcolor(p.t);return \`<div class="chip">
        ...
        <div class="cn">\${p.n.split(' ').slice(-1)[0]}</div>
